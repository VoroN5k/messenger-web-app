...
function lastMsgText(conv: Conversation, t: (k: string) => string): string {
    ...
    if (m.fileUrl) return m.fileType?.startsWith('image/') ? t('message.photo') : t('message.file');
    if (!m.content) return '...';
        ...
        <aside className="w-[340px] bg-white dark:bg-slate-800 border-r border-gray-100 dark:border-slate-700 flex flex-col z-20 shrink-0 transition-colors duration-200">
                ...
                <div className="flex items-center gap-0.5">
                    {pushPermission !== 'unsupported' && (
                        <button
                            ...
                            title={
                                pushPermission === 'granted' ? t('notifications.banner') :
                                    pushPermission === 'denied'  ? 'Заблоковано' : t('notifications.enable')
                            }
            ...
            <div className="flex border-b border-gray-100 dark:border-slate-700">
                {(['chats', 'friends'] as SidebarTab[]).map((tabKey) => (
                    ...
                    >
                        {tabKey === 'chats'
                            ? <><MessageSquare size={14} />{t('sidebar.tab_chats')}</>
                            : <><Users size={14} />{t('sidebar.tab_friends')}
            ...
            <div className="px-3 py-2.5 border-b border-gray-100 dark:border-slate-700">
                <div className="relative">
                    ...
                    <input
                        ...
                        placeholder={tab === 'chats' ? t('sidebar.search_chats') : t('sidebar.search_people')}
                    ...
                    <>
                        <div className="flex gap-1.5 px-3 py-2">
                            ...
                            <button onClick={() => setShowNewGroup(true)}
                                ...
                                <Plus size={12} />{t('sidebar.new_group')}
                            </button>
                            <button onClick={() => setShowNewChan(true)}
                                ...
                                <Hash size={12} />{t('sidebar.new_channel')}
                            </button>
                        </div>
                                            ...
                                            <div className="flex items-center justify-between gap-1">
                                                <div className="flex items-center gap-1 min-w-0">
                                                    ...
                                                    <p className={`font-medium text-sm truncate
                                                        ...
                                                        {isSaved ? t('sidebar.saved') : (conv.name ?? t('chat.default_name'))}
                                                    </p>
                                                </div>
                    ...
                    <>
                                        ...
                                        <div key={u.id} className="flex items-center gap-2 py-2">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-xs text-slate-400 dark:text-slate-500">
                                                    {u.isOnline ? t('status.online') : t('status.offline')}
                                                </p>
                                            </div>
                                            {u.friendshipStatus === 'ACCEPTED' ? (
                                                ...
                                                <span className="text-xs text-slate-400 px-2">{t('sidebar.sent')}</span>
                        ...
                        {friends.length === 0 && searchQuery.trim().length < 2 ? (
                            ...
                            <div className="px-3 py-2">
                                {searchQuery.trim().length < 2 && (
                                    <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-2">
                                        {t('sidebar.friends_count', { count: friends.length })}
                                    </p>
                                )}
                                {friends.map((f) => (
                                    <div key={f.friendshipId} className="flex items-center gap-2 py-2 group">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-slate-400 dark:text-slate-500">
                                                {f.friend.isOnline ? t('status.online') : t('status.offline')}
                                            </p>
                                        </div>
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => openDirect(f.friend.id)}
                                                    className="p-1.5 rounded-full text-slate-400 hover:text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-900/30 cursor-pointer transition-colors"
                                                    title={t('sidebar.write')}>
                                                <MessageSquare size={13} />
                                            </button>
                                            <button onClick={() => onRemoveFriend(f.friend.id)}
                                                    className="p-1.5 rounded-full text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 cursor-pointer transition-colors">
                                                <X size={13} />
                                            </button>
                                        </div>
                                    </div>
...
// CreateGroupModal
// =============================================================================
function CreateGroupModal({ friends, onClose, onCreated, currentUserId }: {
    friends: FriendItem[];
    onClose: () => void;
    onCreated: (c: Conversation) => void;
    currentUserId?: number;
}) {
    const t = useTranslations('group_modal');
    ...
    const submit = async () => {
        ...
        if (!trimmedName)          { setError(t('name_error_empty')); return; }
        if (trimmedName.length < 2){ setError(t('name_error_short')); return; }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm">
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                    ...
                    <button onClick={onClose} className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer text-slate-400">
                        <X size={15} />
                    </button>
                </div>
                <div className="p-5 space-y-4">
                    ...
                    {friends.length === 0 ? (
                        <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-2">{t('no_friends_note')}</p>
                    ) : (
                        <>
                            <p className="text-xs text-slate-400 dark:text-slate-500">{t('select_members')}</p>
                ...
                <div className="px-5 pb-5 flex gap-2">
                    <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors">
                        {t('cancel')}
                    </button>
...
function CreateChannelModal({ onClose, onCreated }: {
    ...
    const t = useTranslations('channel_modal');
    const [name,    setName]    = useState('');
    ...
    const submit = async () => {
        const trimmedName = name.trim();
        if (!trimmedName) { setError(t('name_error')); return; }
        setError(''); setLoading(true);
        try {
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm">
                ...
                <div className="p-5 space-y-3">
                    ...
                    <textarea value={desc} onChange={(e) => setDesc(e.target.value)}
                              placeholder={t('desc_placeholder')} rows={3}
                ...
                <div className="px-5 pb-5 flex gap-2">
                    <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer">
                        {t('cancel')}
                    </button>
                    <button onClick={submit} disabled={loading || !name.trim()}
