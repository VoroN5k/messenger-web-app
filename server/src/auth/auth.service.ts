...
export class AuthService {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!session) {
        ...
        if (reusedSession) {
          ...
          this.logger.warn(
            `Refresh token not found (hash: ${hashed.slice(0, 12)}…)`,
          );
        }
  ...
  async setup2FA(
    userId: number,
  ): Promise<{ secret: string; qrCodeDataUrl: string; manualEntry: string }> {
    const user = await this.prisma.user.findUniqueOrThrow({
      where: { id: userId },
    });
  ...
  async enable2FA(userId: number, token: string): Promise<void> {
    ...
    if (!user.twoFactorSecret)
      throw new BadRequestException(
        '2FA не налаштовано. Спочатку викличте /2fa/setup',
      ); // 2FA not set up. Call /2fa/setup first

    const isValid = authenticator.check(token, user.twoFactorSecret);
    if (!isValid)
      throw new BadRequestException('Невірний код. Перевірте час на пристрої'); // Invalid 2FA token
